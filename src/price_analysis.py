...
df = pd.read_csv(DATA_PATH)
    ...
    # Composite Deal Score: weighted blend of discount, rating, and popularity
    df["deal_score"] = (
        0.5 * df["discount_norm"] + 0.35 * df["rating_norm"] + 0.15 * df["popularity_norm"]
    ).round(3) * 100

    df.to_csv("/home/claude/FlipkartPriceAnalytics/data/flipkart_products_scored.csv", index=False)
    ...
    ).round(2).sort_values("avg_discount_pct", ascending=False)

    brand_summary = df.groupby("brand").agg(
        avg_discount_pct=("discount_pct", "mean"),
        avg_deal_score=("deal_score", "mean"),
        products=("product_id", "count"),
    ).round(2).sort_values("avg_deal_score", ascending=False)

    print("=== Category Summary ===")
    print(category_summary)
    print("\n=== Top 10 Brands by Deal Score ===")
